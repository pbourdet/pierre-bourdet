...
import { getConversations, createMercureEventSource } from '../../../requests/messaging';
...
function ConversationTabs () {
    ...
    useEffect(() => {
        const newEventSource = eventSource ?? createMercureEventSource(process.env.REACT_APP_API_URL + '/conversations');
        ...
        newEventSource.onmessage = function (event) {
            ...
            toast.info(<FormattedMessage id="messaging.create.conversation.other" values={{ name: otherParticipant.user.nickname }}/>);
        };
    ...
    return (
        <>
            <Tab.Container activeKey={activeKey || conversations[0].id} onSelect={(k) => setActiveKey(k)} mountOnEnter>
